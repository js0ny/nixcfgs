...
with lib; {
  ...
  mkDolphinService = name: {
    ...
    actionKeys =
      concatStringsSep ";" (
        if actionOrder != null
        then actionOrder
        else attrNames actions
      )
      + ";";

    mkActionEntry = actionConfig: let
    ...
    actionEntries =
      mapAttrs' (
        actionName: actionConfig:
          nameValuePair "Desktop Action ${actionName}" (mkActionEntry actionConfig)
      )
      actions;
  ...
  in {
    ...
    xdg.dataFile."kio/servicemenus/${name}.desktop".text =
      generators.toINI {} iniContent;
  };
}
